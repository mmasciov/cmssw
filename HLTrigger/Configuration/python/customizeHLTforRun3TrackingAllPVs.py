...
def customizeHLTforRun3TrackingAllPVs(process, allPVsForCutClassifier=True):
    ...
    for producer in producers_by_type(process, "TrackWithVertexSelector"):
        ...
        if not hasatrr(producer,'fractionSumPt2'):
            producer.fractionSumPt2 = cms.double(0.3),
        if not hasatrr(producer,'minSumPt2'):
            producer.minSumPt2 = cms.double(0.0)
        if not hasatrr(producer,'track_chi2_max'):
            producer.track_chi2_max = cms.double(20.0)
        if not hasatrr(producer,'track_pt_max'):
            producer.track_pt_max = cms.double(20.0)
        if not hasatrr(producer,'track_prob_min'):
            producer.track_prob_min = cms.double(-1.0)
        if not hasatrr(producer,'track_pt_min'):
            producer.track_pt_min = cms.double(1.0)
    ...
    if hasattr(process,'hltIter0PFlowTrackCutClassifier'):
        ...
        if not allPVsForCutClassifier:
            process.hltIter0PFlowTrackCutClassifier.vertices = cms.InputTag("hltTrimmedPixelVertices")
